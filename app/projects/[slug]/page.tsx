...
import { getProjectBySlug, getAllProjectSlugs, getAdjacentProjects } from "@/lib/supabase/queries";
...
function GalleryGrid({ items }: { items: any[] }) {
  // We assume items is an array of objects { src, label, aspect, span }
    ...
    <div className="space-y-6">
      {rows.map((row, rowIdx) => {
        ...
        return (
          ...
          >
            {row.map((item, idx) => (
              <ImagePlaceholder key={idx} label={item.label || "Gallery image"} src={item.src} aspectRatio={item.aspect || "video"} />
            ))}
          </div>
        );
      })}
...
export default async function ProjectDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { prevProject, nextProject } = await getAdjacentProjects(project.created_at || "");
      ...
      <main className="pt-32 pb-24 min-h-screen">
        ...
        {project.cover_image && (
          <div className="container mx-auto px-4 md:px-8 max-w-6xl mb-20">
            <ImagePlaceholder label={`${project.title} — Hero`} src={project.cover_image} aspectRatio="video" className="rounded-2xl" />
          </div>
        )}
        ...
        <div className="container mx-auto px-4 md:px-8 max-w-5xl mb-20">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-6 md:p-8 rounded-2xl bg-muted/30 border border-border">
            <div>
              ...
              <p className="font-heading font-bold text-lg">{project.category || "General"}</p>
            </div>
            {techStack.length > 0 && (
              <div className="col-span-2">
                <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Tools</h4>
                <p className="font-heading font-bold text-lg">{techStack.slice(0, 3).join(", ")}</p>
              </div>
            )}
            {(project.live_url || project.github_url) && (
              <div>
                <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">Links</h4>
                <div className="flex gap-4">
                  {project.live_url && (
                    <a href={project.live_url} target="_blank" rel="noreferrer" className="text-foreground hover:text-muted-foreground transition-colors">
                      <Globe className="h-5 w-5" />
                    </a>
                  )}
                  {project.github_url && (
                    <a href={project.github_url} target="_blank" rel="noreferrer" className="text-foreground hover:text-muted-foreground transition-colors">
        ...
        {project.full_description && (
          <div className="container mx-auto px-4 md:px-8 max-w-4xl mb-20">
